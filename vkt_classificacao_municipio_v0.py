...
df_vkt = pd.read_csv('datasets/2020/vkt_v6.csv', usecols=['Modelo_iCarros_A', 'Modelo_iCarros_B', 'Tipo', 'Marca_Modelo', 'Ano', 'Quantidade'], low_memory=False)
print(df_vkt)
...
#df_vkt = df_vkt.groupby(['Marca_Modelo', 'Ano', 'Tipo', 'Modelo_iCarros_A', 'Modelo_iCarros_B'])['Quantidade'].sum().sort_values(ascending=False)
#df_vkt = df_vkt[df_vkt['Quantidade'] > np.percentile(df_vkt['Quantidade'],95)]
#vkt2 = np.percentile(df_vkt['Quantidade'],95)
#print(vkt2)
print(df_vkt)
df_vkt.to_csv('datasets/2020/vkt_classificacao_v9.csv')
