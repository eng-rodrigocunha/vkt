...
marcas_modelos3.sort_values(['Consumo_Cidade_Gasolina'], inplace=True)
...
marcas_chaveB_modelos4_lista = marcas_modelos4['Chave_B'].tolist()
marcas_chaveB_modelos4_lista = list(set(marcas_chaveB_modelos4_lista))
...
for index, row in marcas_modelos3.iterrows():
    #Scraping de Versões do Veículo
    try:
        if(index > 5):
            break

        #Carregamento completo da Página
        time.sleep(random.randrange(start=0, stop=3))
